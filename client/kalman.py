...
class Kalman:
    ...
    def computeAndUpdateRollPitchYaw(self, ax, ay, az, gx, gy, gz, mx, my, mz, dt):
        ...
        if not reset:
            self.roll, self.currentRollState, self.rollCovariance = self.update(
                self.currentRollState,
                measuredRoll,
                self.rollCovariance,
                self.rollError,
                self.rollDriftError,
                self.rollMeasurementError,
                gx,
                dt,
            )

        self.pitch, self.currentPitchState, self.pitchCovariance = self.update(
            self.currentPitchState,
            measuredPitch,
            self.pitchCovariance,
            self.pitchError,
            self.pitchDriftError,
            self.pitchMeasurementError,
            gy,
            dt,
        )

        self.yaw, self.currentYawState, self.yawCovariance = self.update(
            self.currentYawState,
            measuredYaw,
            self.yawCovariance,
            self.yawError,
            self.yawDriftError,
            self.yawMeasurementError,
            gz,
            dt,
        )
    ...
    def __restrictRollAndPitch(self, measuredRoll, measuredPitch, gy):
        ...
        reset = 0
        if (measuredRoll < -90 and self.roll > 90) or (
            measuredRoll > 90 and self.roll < -90
        ):
    ...
    def computeRollAndPitch(self, ax, ay, az):
        ...
        measuredPitch = np.degrees(
            np.arctan2(-1 * ax, np.sqrt(np.square(ay) + np.square(az)))
        )
    ...
    def computeYaw(self, roll, pitch, mx, my, mz):
        ...
        measuredYaw = np.degrees(
            np.arctan2(
                np.sin(roll) * mz - np.cos(roll) * my,
                np.cos(pitch) * mx
                + np.sin(roll) * np.sin(pitch) * my
    ...
    def update(
        ...
        prediction = np.matmul(motionModel, currentState) + dt * (
            np.vstack((angularVelocity, 0.0))
        )

        errorMatrix = np.array([error, driftError]) * np.identity(2)
        predictedCovariance = (
            np.matmul(np.matmul(motionModel, currentCovariance), (motionModel.T))
            + errorMatrix
        )
        ...
        measurementCovariance = (
            np.matmul(
                np.matmul(np.array([1.0, 0.0]), predictedCovariance),
                np.vstack((1.0, 0.0)),
            )
            + measurementError
        )
        kalmanGain = (
            np.matmul(predictedCovariance, np.vstack((1.0, 0.0)))
            / measurementCovariance
        )

        correctedState = prediction + kalmanGain * (
            measurement - np.matmul(np.array([1.0, 0.0]), prediction)
        )
        # correctedState = prediction + kalmanGain*(difference)

        updatedCovariance = np.matmul(
            np.identity(2)
